...
child1: 						#Execing for the first process with linear search
								la $a0,process_name_1
								li $v0, 20
								syscall

child2: 						#Execing for the first process with binary search
								la $a0,process_name_2
								li $v0, 20
								syscall

child3: 						#Execing for the first process with collatz
								la $a0,process_name_3
								li $v0, 20
								syscall
		
child4: 						#Execing for the first process with palindrome
								la $a0,process_name_4
								li $v0, 20
								syscall

exit:							#Wait pid for the first child
								li $v0, 23
								move $a0, $s0
								syscall

								#Wait pid for the second child
								li $v0, 23
								move $a0, $s1
								syscall

								#Wait pid for the third child
								li $v0, 23
								move $a0, $s2
								syscall

								#Wait pid for the fourth child
								li $v0, 23
								move $a0, $s3
								syscall
